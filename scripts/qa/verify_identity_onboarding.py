...
def main() -> int:
    ...
    require_text(root, "backend/portal/serializers.py", ["required=False", "allow_null=True"])
    require_text(
        root,
    ...
    require_text(root, "backend/administration/urls.py", ['"self-service/companies/"', "self_service_company_create_view"])
    require_text(root, "backend/administration/views.py", ["def self_service_company_create_view(request):", 'code="CASA"', 'name="Casa Matriz"'])
    require_text(root, "frontend/src/app/pages/login/login.ts", ["? returnUrl : '/portal'"])
    require_text(root, "frontend/src/app/pages/portal-register/portal-register.ts", ["firstName", "lastName", "email", "password", "navigate(['/portal'])"])

    register_html = require_file(root, "frontend/src/app/pages/portal-register/portal-register.html").read_text(encoding="utf-8-sig")
    ...
    require_text(
        ...
        ["createOwnCompany(", "organizationContext.load()", "selectMembership(", "navigate(['/app/dashboard'])"],
    )
    require_text(root, "frontend/src/app/layouts/app-shell/app-shell.html", ['routerLink="/portal"', "Ver como cliente"])
    require_text(root, "frontend/src/app/pages/portal-account/portal-account.html", ['routerLink="/portal/seller-onboarding"', "Crear mi PYME", "Gestionar mi PYME"])

    routes = require_file(root, "frontend/src/app/app.routes.ts").read_text(encoding="utf-8-sig")
    ...
    maintainers_specs = sorted((root / "frontend/projects/maintainers/src").rglob("*.spec.ts"))
    ...
    require_text(root, "README.md", ["persona primero", "Crear mi PYME", "sin cerrar sesión"])
